...
namespace engine
{
	...
	D3D12HelloConstBuffers::D3D12HelloConstBuffers(UINT width, UINT height, std::string name) :
	...
	void D3D12HelloConstBuffers::LoadPipeline()
	{
		...
		ThrowIfFailed(factory->MakeWindowAssociation(MainEngine::Get().GetHWND(), DXGI_MWA_NO_ALT_ENTER));
